...
const initSocket = (server) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on(SOCKET_EVENTS.JOIN, (userId) => {
      ...
      if (process.env.NODE_ENV === "development")
        console.log(
          `[Socket] User ${uid} joined. Online users: ${onlineUsers.size}`,
        );
    ...
    socket.on(
      ...
      async ({ chatId, text, recipientId, tempId }) => {
        try {
          ...
          io.to(`user:${recipientId}`).emit(SOCKET_EVENTS.NEW_MESSAGE, {
            chatId,
            message,
          });

          const chat = await require("../models/Chat")
            .findById(chatId)
    ...
    socket.on(
      ...
      async ({ chatId, messageId, senderId }) => {
        try {
          const recipientId = socket.userId;
          if (!recipientId) return;

          const chatService = require("../services/chatService");
          const message = await chatService.markDelivered(
            chatId,
            messageId,
            recipientId,
          );
    ...
    socket.on(SOCKET_EVENTS.TYPING_START, ({ chatId, recipientId }) => {
      ...
      io.to(`user:${recipientId}`).emit(SOCKET_EVENTS.USER_TYPING, {
        chatId,
        userId: senderId,
      });
    });
    ...
    socket.on(SOCKET_EVENTS.TYPING_STOP, ({ chatId, recipientId }) => {
      const senderId = socket.userId;
      if (!senderId || !recipientId) return;
      io.to(`user:${recipientId}`).emit(SOCKET_EVENTS.USER_STOPPED_TYPING, {
        chatId,
        userId: senderId,
      });
    });
    ...
    socket.on(
      SOCKET_EVENTS.INITIATE_CALL,
      ({ targetUserId, roomId, callerName, callerAvatar }) => {
        const callerId = socket.userId;
        if (!callerId || !targetUserId) return;

        console.log(
          `[Socket] Call initiated by ${callerId} to ${targetUserId}, room: ${roomId}`,
        );
    ...
    socket.on(
      ...
      ({ targetUserId, requesterName, chatId }) => {
        const requesterId = socket.userId;
        if (!requesterId || !targetUserId) return;

        console.log(
          `[Socket] Call requested by student ${requesterId} to tutor ${targetUserId}`,
        );
