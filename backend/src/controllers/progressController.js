...
exports.markLessonComplete = asyncHandler( async (req,res) =>{
    ...
    const data = await progressService.markLessonComplete(req.user.id, courseId, lessonId);
    res.status(HTTP_STATUS.OK).json({ success: true, message: 'Lesson marked as complete', data });
});
