...
exports.createLesson = asyncHandler(async (req, res) => {
  ...
  res
    ...
    .json({ success: true, message: "Lesson created successfully", data });
});
...
exports.updateLesson = asyncHandler(async (req, res) => {
  ...
  res
    ...
    .json({ success: true, message: "Lesson updated successfully", data });
});
...
exports.reorderLessons = asyncHandler(async (req, res) => {
  const data = await lessonService.reorderLessons(
    req.params.courseId,
    req.user.id,
    req.body.lessons,
  );
  res
    .status(HTTP_STATUS.OK)
    .json({ success: true, message: "Lessons reordered successfully", data });
});

exports.getLessonPdf = asyncHandler(async (req, res) => {
  const fileUrl = await lessonService.getPdfFilePath(
    req.params.id,
    req.user.id,
  );
