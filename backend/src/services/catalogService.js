...
const catalogService = {
  ...
  async getCourseDetails(courseId, userId = null) {
    ...
    const course = await Course.findById(courseId).populate(
      "tutor",
      "name email profileImage tutorProfile",
    );
    ...
    const isEnrolled =
      userId &&
      course.studentsEnrolled.some((id) => id.toString() === userId.toString());
    ...
    const tutorCourses = await Course.find({ tutor: course.tutor._id }).select(
      "studentsEnrolled",
    );
    const uniqueStudentIds = new Set();
    tutorCourses.forEach((c) => {
      (c.studentsEnrolled || []).forEach((id) =>
    ...
    courseObj.totalDuration = lessons.reduce(
