...
const lessonService = {
  async createLesson(courseId, tutorId, lessonData, files = {}) {
    ...
    if (existingLesson) {
      throw new Error(
        `Lesson with order ${order} already exists for this course`,
      );
    }
    ...
    const enrolledCourse = await Course.findById(courseId).select(
      "studentsEnrolled title",
    );
  ...
  async getLessonById(lessonId, userId = null, userRole = null) {
    ...
    const isEnrolled =
      userId && lesson.course.studentsEnrolled.includes(userId);
  ...
  async getPdfFilePath(lessonId, userId) {
    ...
    const isEnrolled = lesson.course.studentsEnrolled
      .map((s) => s.toString())
      .includes(userId);
