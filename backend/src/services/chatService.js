...
const chatService = {
    ...
    async createOrGetChat(userId1, userId2) {
        ...
        const isAuthorized = await this.validateEnrollmentRelationship(studentId, tutorId);
        if (!isAuthorized) {
            throw new Error('Chat is only available for students enrolled in this tutor\'s courses');
        }
    ...
    async markMessagesRead(chatId, readerId) {
        ...
        chat.messages.forEach(msg => {
            ...
            const alreadyRead = msg.readBy.some(id => id.toString() === readerId.toString());
    ...
    async getUserChats(userId) {
        ...
        return chats.map(chat => ({
            ...
            unreadCount: chat.messages.filter(
                m => !m.readBy.some(id => id.toString() === userId.toString())
    ...
    async getEligibleContacts(userId) {
        ...
        if (user.role === USER_ROLES.STUDENT) {
            const courses = await Course.find({ studentsEnrolled: userId })
                .populate('tutor', 'name email profileImage role tutorProfile.bio');
        ...
        if (user.role === USER_ROLES.TUTOR) {
            const courses = await Course.find({ tutor: userId })
                .populate('studentsEnrolled', 'name email profileImage role');
