...
const quizService = {
  async createQuiz(tutorId, quizData) {
    ...
    if (existingQuiz) {
      ...
      if (quizData.maxAttempts !== undefined)
        updateSet.maxAttempts = quizData.maxAttempts;
    ...
    const cleanQuestions = quizData.questions.map(
      ({ _id, __v, ...rest }) => rest,
    );
  ...
  async updateQuiz(tutorId, quizId, updateData) {
    ...
    if (updateSet.questions) {
      updateSet.questions = updateSet.questions.map(
        ({ _id, __v, ...rest }) => rest,
      );
    }
  ...
  async getStudentQuizStatus(studentId, courseId) {
    ...
    if (attempts.length > 0) {
      ...
      const attemptsToday = attempts.filter(
        (a) => new Date(a.createdAt) >= startOfDay,
      );
      ...
      if (lastAttempt.status === QUIZ_STATUS.STARTED) {
        const expectedEndTime =
          new Date(lastAttempt.startTime).getTime() + quiz.duration * 60 * 1000;
      ...
      if (
        !lastAttempt.passed &&
        (lastAttemptSubmittedAt || lastAttemptStatus !== QUIZ_STATUS.STARTED)
      ) {
        const timeSinceLastAttempt =
          Date.now() -
          new Date(lastAttemptSubmittedAt || lastAttempt.updatedAt).getTime();
        ...
        if (attemptsTodayCount >= quiz.maxAttempts) {
          ...
          if (timeSinceLastAttempt < lockPeriod) {
            ...
            cooldownEndsAt = new Date(
              new Date(
                lastAttemptSubmittedAt || lastAttempt.updatedAt,
              ).getTime() + lockPeriod,
            );
          }
        } else {
  ...
  async startAttempt(studentId, quizId) {
    ...
    const status = await this.getStudentQuizStatus(
      studentId,
      quiz.courseId.toString(),
    );

    if (!status.isCourseCompleted)
      throw new Error(
        "You must complete all lessons before starting the quiz.",
      );

    const ongoingAttempt = status.attempts.find(
      (a) => a.status === QUIZ_STATUS.STARTED,
    );
    if (ongoingAttempt) {
      const expectedEndTime =
        new Date(ongoingAttempt.startTime).getTime() +
    ...
    if (status.cooldownActive)
      throw new Error("You are currently in a cool-down period. Please wait.");
    if (status.attemptsTodayCount >= quiz.maxAttempts)
  ...
  async autosaveAttempt(studentId, attemptId, answers) {
    ...
    if (attempt.status !== QUIZ_STATUS.STARTED) {
      throw new Error(
        "Cannot save answers. The quiz is already submitted or timed out.",
      );
    }
  ...
  async submitAttempt(studentId, attemptId, reason = "submitted") {
    ...
    attempt.answers.forEach((answer) => {
      ...
      const snapshotQuestion = attempt.questionsSnapshot.find(
        (q) => q.questionId.toString() === answer.questionId.toString(),
      );
      if (originalQuestion && snapshotQuestion) {
        if (
          answer.selectedOptionIndex === snapshotQuestion.correctOptionIndex
        ) {
