...
const walletService = {
  ...
  async creditFromOrder(order) {
    const releaseAt = new Date(
      Date.now() + TUTOR_HOLD_DAYS * 24 * 60 * 60 * 1000,
    );

    // Tutor hold
    for (const item of order.courses) {
      const tutorWallet = await walletService.getOrCreateWallet(
        item.tutor,
        "tutor",
      );
      tutorWallet.totalEarnings = round(
        tutorWallet.totalEarnings + item.tutorShare,
      );
    ...
    if (adminUser) {
      const adminWallet = await walletService.getOrCreateWallet(
        adminUser._id,
        "admin",
      );
      ...
      adminWallet.totalEarnings = round(
        adminWallet.totalEarnings + order.finalAmount,
      );
  ...
  async reverseEarning(tutorId, orderRef, amount) {
    ...
    if (!transaction)
      throw new Error("Earning transaction not found for this order");
    if (
  ...
  async reverseAdminEscrow(orderRef, fullAmount) {
    ...
    if (isStillHeld) {
      ...
      transaction.status = "refunded";
      const commission =
        transaction.platformShare != null
  ...
  async getWallet(userId, { page = 1, limit = 10, type } = {}) {
    ...
    let txns = [...freshWallet.transactions].sort(
      (a, b) => new Date(b.createdAt) - new Date(a.createdAt),
    );
  ...
  async getWalletWithHold(userId, ownerType) {
    ...
    const pendingAmount = round(
      freshWallet.transactions
        .filter(
          (t) => t.status === "pending" && t.releaseAt && t.releaseAt > now,
        )
  ...
  async requestWithdrawal(tutorId, amount) {
    ...
    const availableBalance = round(
      freshWallet.balance - pendingWithdrawalAmount,
    );
      ...
      throw new Error(
        `Insufficient available balance. Available: ₹${availableBalance.toFixed(2)}` +
          (pendingWithdrawalAmount > 0
            ? ` (₹${pendingWithdrawalAmount.toFixed(2)} pending approval)`
            : ""),
      );
    }
  ...
  async approveWithdrawal(requestId, adminId, adminNote = "") {
    ...
    return {
      request,
      message: `Withdrawal of ₹${request.amount} approved for ${request.tutor.name}`,
    };
  },
  ...
  async rejectWithdrawal(requestId, adminId, adminNote = "") {
    ...
    return {
      request,
      message: `Withdrawal request rejected for ${request.tutor.name}`,
    };
  },
};
