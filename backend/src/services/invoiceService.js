  ...
  generateInvoicePDF(order, res) {
    ...
    metaRows.forEach(([label, value, highlight]) => {
      doc
        ...
        .fontSize(8)
        .fillColor(GRAY)
        .text(label, metaLabelX, mY);
      doc
        .font(highlight ? FONT_BOLD : FONT_REGULAR)
    ...
    doc.text("ORIGINAL", COL_ORIG, tblY + 9, {
      align: "right",
      width: COL_ORIG_W,
    });
    doc.text("DISCOUNT", COL_DISC, tblY + 9, {
      align: "right",
      width: COL_DISC_W,
    });
    ...
    order.courses.forEach((item, idx) => {
      ...
      doc
        ...
        .fontSize(9)
        .fillColor(GRAY)
        .text(INR(original), COL_ORIG, rowY, {
          align: "right",
          width: COL_ORIG_W,
        });
      ...
      doc
        ...
        .fontSize(10)
        .fillColor(DARK)
        .text(INR(discounted), COL_AMT, rowY, {
          align: "right",
          width: COL_AMT_W,
        });
    ...
    const sumRow = (label, value, bold = false, color = DARK) => {
      doc
        ...
        .fontSize(9)
        .fillColor(GRAY)
        .text(label, sumLX, curSumY);
      doc
        .font(bold ? FONT_BOLD : FONT_REGULAR)
        .fontSize(9)
        .fillColor(color)
        .text(value, sumLX, curSumY, { align: "right", width: sumW });
      curSumY += 18;
    };
    ...
    curSumY += 4;
    doc
      .moveTo(sumLX, curSumY)
      .lineTo(PW - MR, curSumY)
      .strokeColor(BORDER)
      .lineWidth(0.5)
      .stroke();
    curSumY += 8;
