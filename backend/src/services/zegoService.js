...
function generateToken04(appId, userId, secret, effectiveTimeInSeconds, payload = '') {
    if (!appId || typeof appId !== 'number') throw new Error('ZegoCloud: Invalid appId');
    if (!userId) throw new Error('ZegoCloud: userId is required');
    if (!secret || secret.length !== 32) throw new Error('ZegoCloud: Server secret must be 32 characters');
...
const zegoService = {
    async generateCallToken(chatId, userId, existingRoomId = null) {
        ...
        if (!chat) {
            throw new AppError('You are not a participant in this chat', HTTP_STATUS.FORBIDDEN);
        }
