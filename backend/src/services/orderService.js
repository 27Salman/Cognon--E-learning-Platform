...
const orderService = {
  ...
  async getAllOrders({
    ...
    if (search && search.trim()) {
      query.$or = [{ orderId: { $regex: search.trim(), $options: "i" } }];
    }
  ...
  async cancelCourse(userId, orderId) {
    ...
    const daysSincePurchase =
      (Date.now() - new Date(order.orderDate)) / (1000 * 60 * 60 * 24);
    if (daysSincePurchase > 3)
      throw new Error("Refund window has expired (3 days from purchase)");
    ...
    for (const courseItem of order.courses) {
      await walletService.reverseEarning(
        courseItem.tutor,
        order._id,
        courseItem.tutorShare,
      );
    }

    await walletService.reverseAdminEscrow(order._id, order.finalAmount);

    await walletService.refundToStudent(
      userId,
      order.finalAmount,
      order.orderId,
    );
    ...
    await User.findByIdAndUpdate(userId, {
      $pull: {
        "studentProfile.enrolledCourses": { courseId: { $in: courseIds } },
      },
    });

    let latestRestriction = await CourseRestrict.findOne({
      userId,
    ...
    return {
      message: "Course cancelled and refund credited to your wallet",
      refundAmount: order.finalAmount,
    };
  },
};
