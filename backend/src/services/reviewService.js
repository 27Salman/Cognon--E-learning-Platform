...
const reviewService = {
    ...
    async deleteReview(courseId, studentId) {
        const review = await Review.findOneAndDelete({ course: courseId, student: studentId });
    ...
    async getTutorCourseReviews(courseId, tutorId, { page = 1, limit = 10 } = {}) {
        const course = await Course.findOne({ _id: courseId, tutor: tutorId }).select('_id');
        if (!course) {
            const err = new Error('Course not found or you are not authorised to view these reviews');
            err.statusCode = HTTP_STATUS.FORBIDDEN;
            throw err;
        }
