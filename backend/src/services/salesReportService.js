...
const salesReportService = {
  generateSalesPDF(reportData, res) {
    ...
    const FONT_REGULAR = pathMod.join(
      __dirname,
      "../../assets/fonts/Roboto-Regular.ttf",
    );
    const FONT_BOLD = pathMod.join(
      __dirname,
    ...
    const cards = [
      ...
      {
        label: "Tutor Payouts",
        value: INR(summary.totalTutorRevenue),
        highlight: false,
      },
      {
        label: "Platform Revenue",
        value: INR(summary.totalPlatformRevenue),
        highlight: false,
      },
      {
        label: "Total Orders",
      ...
      {
        label: "Razorpay Orders",
        value: String(summary.paymentMethods?.razorpay?.count || 0),
        highlight: false,
      },
      {
        label: "Wallet Orders",
        value: String(summary.paymentMethods?.wallet?.count || 0),
        highlight: false,
      },
    ];
    ...
    cards.forEach((card, i) => {
      ...
      doc
        ...
        .fill(card.highlight ? PURPLE_MID : LIGHT_GRAY);
      doc
        .font(FONT_REGULAR)
        .fontSize(8)
    ...
    summarySheet.addRows([
      {
        ...
        value: dateFrom && dateTo ? `${dateFrom} to ${dateTo}` : "All Time",
      },
      {
        metric: "Total Revenue (Rs.)",
        value: Math.round(summary.totalRevenue),
      },
      {
        metric: "Tutor Payouts (Rs.)",
        value: Math.round(summary.totalTutorRevenue),
      },
      {
        metric: "Platform Revenue (Rs.)",
        value: Math.round(summary.totalPlatformRevenue),
      },
      { metric: "Total Orders", value: summary.totalOrders },
      {
        metric: "Razorpay Orders",
        value: summary.paymentMethods?.razorpay?.count || 0,
      },
      {
        metric: "Razorpay Revenue (Rs.)",
        value: Math.round(summary.paymentMethods?.razorpay?.revenue || 0),
      },
      {
        metric: "Wallet Orders",
        value: summary.paymentMethods?.wallet?.count || 0,
      },
      {
        metric: "Wallet Revenue (Rs.)",
        value: Math.round(summary.paymentMethods?.wallet?.revenue || 0),
      },
    ]);
  ...
  generateTutorPDF(reportData, tutorName, res) {
    const { summary, courseBreakdown, transactions, dateFrom, dateTo } =
      reportData;
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=tutor-sales-${Date.now()}.pdf`,
    );
    doc.pipe(res);
    ...
    const cards = [
      {
        label: "Your Earnings",
        value: INR(summary.totalEarnings),
        highlight: true,
      },
      {
        label: "Gross Revenue",
  ...
  async generateTutorExcel(reportData, tutorName, res) {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename=earnings-report-${Date.now()}.xlsx`,
    );
    await workbook.xlsx.write(res);
    res.end();
  },
