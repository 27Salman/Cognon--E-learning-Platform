...
export default function QuizOpen({ courseId, courseProgress }) {
  ...
  if (loading)
    return (
      <div className="animate-pulse bg-gray-200 h-24 rounded-xl w-full mt-6"></div>
    );
    ...
    return (
      <div className="mt-4 mb-2">
        <div className="bg-green-50 border border-green-200 rounded-xl p-3 relative overflow-hidden">
          ...
          <p className="text-xs text-green-700 mt-1 pl-2">
            Score: <span className="font-bold">{passedAttempt.score}</span> /{" "}
            {quiz.questions?.reduce((acc, q) => acc + (q.marks || 1), 0) ||
              quiz.totalQuestions ||
              0}
          </p>
          <button
    ...
    <>
      <div className="mt-4 mb-2">
        <div className="bg-white border border-purple-200 shadow-sm rounded-xl p-3 relative overflow-hidden group hover:border-purple-300 transition-colors">
          ...
          <h3
            className="text-sm font-bold text-gray-800 mb-2 pl-2 truncate"
            title={quiz.title}
          >
            {quiz.title}
          </h3>
          ...
          <div className="grid grid-cols-2 gap-y-2 gap-x-1 text-xs text-gray-600 mb-3 pl-2">
            ...
            <div className="col-span-2 flex items-center justify-between bg-gray-50 px-2 py-1 rounded-md border border-gray-100">
              ...
              <span
                className={`font-bold ${attemptsTodayCount >= quiz.maxAttempts ? "text-red-500" : "text-purple-600"}`}
              >
          ...
          <div className="px-1">
            {!isCourseCompleted ? (
              ...
              >
                ...
                {attempts?.some((a) => a.status === "started")
                  ? "Resume Quiz"
                  : "Start Exam"}
              </button>
            )}
          ...
          <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl relative">
            ...
            <div className="bg-purple-50 p-4 rounded-xl mb-4">
              <ul className="text-sm text-gray-700 space-y-2 list-disc pl-4">
                <li>
                  The exam will run for{" "}
                  <span className="font-bold">{quiz.duration} minutes</span>.
                </li>
                <li>
                  The timer <span className="font-bold">cannot be paused</span>{" "}
                  once started.
                </li>
                <li>
                  Do <span className="font-bold text-red-600">NOT</span> switch
                  tabs or exit fullscreen, or your exam may be auto-submitted.
                </li>
