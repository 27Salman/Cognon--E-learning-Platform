...
function RevenueList({ onCourseClick }) {
  ...
  if (sort === "revenue_desc")
    courses = [...courses].sort((a, b) => b.totalRevenue - a.totalRevenue);
  else if (sort === "revenue_asc")
    courses = [...courses].sort((a, b) => a.totalRevenue - b.totalRevenue);
  else if (sort === "students_desc")
    courses = [...courses].sort((a, b) => b.enrolledCount - a.enrolledCount);
  else if (sort === "newest")
    ...
    <div className="p-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-6">
        {[
          ...
          <div
            key={label}
            className={`rounded-xl border border-gray-200 p-4 ${color.split(" ")[0]}`}
          >
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-4 mb-6">
        ...
        {!dateFrom && !dateTo && (
          <p className="text-xs text-gray-400 mt-2">
            No date filter — downloads all-time report
          </p>
        )}
      </div>
      {/* Search + Sort */}
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="border-b border-gray-100">
            <tr>
              <th className="text-left px-5 py-3 text-gray-500 font-semibold text-xs w-12">
                S1
              </th>
              <th className="text-left px-5 py-3 text-gray-500 font-semibold text-xs">
                Course Name
              </th>
              <th className="text-left px-5 py-3 text-gray-500 font-semibold text-xs">
                Students
...
function RevenueDetails({ course, onBack }) {
  ...
  const fetchDetails = async (p = 1, s = "") => {
    setLoading(true);
    try {
      const res = await tutorAPI.getCourseRevenueDetails(course._id, {
        page: p,
        limit: LIMIT,
        search: s,
      });
