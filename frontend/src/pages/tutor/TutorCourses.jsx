...
function StatusToggle({ course, onToggle }) {
  ...
  const handleToggle = async () => {
    ...
    try {
      await onToggle(
        course._id,
        isListed ? COURSE_STATUS.DRAFT : COURSE_STATUS.PUBLISHED,
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="flex items-center gap-2 mt-2">
      <span
        className={`text-sm font-medium ${isListed ? "text-purple-600" : "text-red-400"}`}
      >
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {courses.map((course) => (
            <div
              key={course._id}
              className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 flex items-center gap-5"
            >
              {/* Thumbnail - clickable */}
              <div
      ...
      {pagination.totalPages > 1 && (
        <div className="flex items-center justify-between mt-6">
          <p className="text-sm text-gray-500">
            ...
            {Math.min(currentPage * LIMIT, pagination.totalCourses)} of{" "}
