...
export default function EditCourse() {
  ...
  const handleAddLesson = async () => {
    ...
    if (!lessonForm.chapterTitle?.trim())
      return toast.error("Chapter title required");
    ...
    try {
      ...
      if (editingLesson) {
        const res = await courseAPI.updateLesson(
          editingLesson._id,
          formData,
          config,
        );
        ...
        setLessons((prev) =>
          prev.map((l) => (l._id === editingLesson._id ? updated : l)),
        );
      ...
      const errorMsg =
        data?.errors?.[0]?.message || data?.message || "Failed to save lesson";
          ...
          <div>
            ...
            {form.offerPercentage > 0 && form.price > 0 && (
              <p className="text-xs text-green-600 mt-1">
                Discounted price: ₹
                {Math.round(form.price * (1 - form.offerPercentage / 100))}
              </p>
            )}
          </div>
          <div>
      ...
      <div className="bg-purple-50 border border-purple-200 rounded-xl p-4 mb-6">
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
          ...
          <input
            ...
            onChange={(e) =>
              setLessonForm((p) => ({ ...p, chapterTitle: e.target.value }))
            }
          ...
          <input
            ...
            type="number"
            min="1"
            value={lessonForm.chapterOrder}
            onChange={(e) =>
              setLessonForm((p) => ({ ...p, chapterOrder: e.target.value }))
            }
            className="border border-purple-200 bg-white rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-purple-400"
          />
            ...
            <label className="flex items-center justify-between cursor-pointer bg-white border border-purple-200 px-3 py-2 rounded-lg hover:bg-purple-50 transition-colors w-full text-sm text-gray-500">
              <span className="flex items-center gap-2">
                ...
                {lessonVideo
                  ? lessonVideo.name
                  : lessonForm.videoUrl
                    ? "Replace existing video (MP4, MOV)"
          ...
          <div>
            ...
            <label className="block w-full h-24 border-2 border-dashed border-purple-300 bg-white rounded-lg cursor-pointer hover:bg-purple-50 transition-colors">
              <div className="flex flex-col items-center justify-center h-full text-purple-400">
                <Upload className="w-5 h-5 mb-1" />
                <p className="text-xs">
                  {lessonPdf ? lessonPdf.name : "Upload PDF"}
                </p>
              </div>
              <input
                type="file"
