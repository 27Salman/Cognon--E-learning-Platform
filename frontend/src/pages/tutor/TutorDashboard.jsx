...
export default function TutorDashboard() {
  ...
  const defaultWeekly = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(
    (day) => ({
      day,
      students: 0,
      revenue: 0,
  ...
  const allMonthlyChartData = monthlyData.map((m) => {
    ...
    const label = mon
      ? new Date(Number(year), Number(mon) - 1, 1).toLocaleString("default", {
          month: "short",
          year: "2-digit",
        })
      : m.month;
      ...
      {showPendingWarning && (
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-5">
          ...
          <p className="text-sm text-amber-600 mt-0.5">
            Your tutor account is awaiting admin approval before you can publish
            courses.
          </p>
        </div>
      )}
            ...
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                {[
                  ...
                  <th
                    key={h}
                    className="text-left px-4 py-3 text-gray-600 font-semibold whitespace-nowrap text-xs"
                  >
                    {h}
                  </th>
                ))}
