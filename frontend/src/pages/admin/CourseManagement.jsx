  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow">
      ...
      <div className="relative">
        ...
        <span
          className={`absolute top-2 right-2 px-2 py-0.5 rounded-full text-xs font-medium capitalize ${STATUS_COLORS[course.status] || "bg-gray-100 text-gray-600"}`}
        >
...
export default function AdminCourseManagement() {
  ...
  const loadCourseReviews = async (courseId, page = 1, replace = false) => {
    ...
    try {
      const res = await courseAPI.getCourseReviews(courseId, {
        page,
        limit: 5,
      });
      const data = res?.data || {};
  ...
  const handleToggleListing = async (courseId, currentStatus) => {
    ...
    try {
      ...
      toast.success(
        `Course ${newStatus === COURSE_STATUS.PUBLISHED ? "listed" : "unlisted"} successfully`,
      );
      if (selectedCourse?._id === courseId) {
        setSelectedCourse((prev) => ({ ...prev, status: newStatus }));
      }
      categories.forEach((cat) =>
        loadCategoryPage(cat.name, categoryCourses[cat.name]?.page || 1),
      );
    } catch (err) {
                          ...
                          >
                            ...
                            <div className="divide-y divide-gray-100">
                              {chapter.lessons.map((lesson, idx) => (
                                <div
                                  key={lesson._id || idx}
                                  className="flex items-center justify-between px-4 py-3 bg-white hover:bg-gray-50 transition text-sm"
                                >
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                      {visibleLessons.map((lesson, i) => (
                        ...
                        >
                          ...
                          <div className="p-3">
                            <p className="text-xs font-semibold text-gray-700 line-clamp-2 mb-1">
                              {lesson.title}
                            </p>
                            <div className="flex items-center justify-between">
                              {lesson.duration && (
                        ...
                        <div className="space-y-3 text-sm">
                          ...
                          {selectedLesson.videoUrl && (
                            <div>
                              ...
                              {selectedLesson.videoUrl.includes(
                                "youtube.com",
                              ) ||
                ...
                <div className="space-y-3">
                  {reviews.length > 0 ? (
                    reviews.map((review) => (
                      <div
                        key={review._id}
                        className="p-4 bg-white rounded-xl border border-gray-150 shadow-sm space-y-2"
                      >
                        <div className="flex items-center gap-3">
                ...
                {selectedCourse.offerPercentage > 0 ? (
                  <>
                    <span className="text-lg font-bold text-gray-800">
                      ...
                      {Math.round(
                        selectedCourse.price *
                          (1 - selectedCourse.offerPercentage / 100),
                      )}
                    </span>
                    ...
                    <span className="text-xs text-green-600 font-medium">
                      {selectedCourse.offerPercentage}% off
                    </span>
                  </>
                ) : (
                  <span className="text-lg font-bold text-gray-800">
