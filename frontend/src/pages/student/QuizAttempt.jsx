...
export default function QuizAttempt() {
    ...
    useEffect(() => {
        const initQuiz = async () => {
            try {
                ...
                const endTime = new Date(currentAttempt.startTime).getTime() + (statusData.quiz.duration * 60 * 1000);
        ...
        const handleFocusLoss = () => {
            setViolationCount(prev => {
                ...
                if (newCount >= maxViolations) {
                    ...
                    toast.error('Exam automatically submitted due to multiple rule violations.');
                } else {
                    setShowWarning(true);
                    toast.error(`Warning ${newCount}/${maxViolations}: Please stay on the exam tab!`);
                }
                return newCount;
            });
        };
    ...
    if (loading) return <div className="h-screen flex items-center justify-center text-gray-500">Preparing your exam...</div>;
        ...
        <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-red-900/90 backdrop-blur-sm">
                    <div className="bg-white p-8 rounded-2xl max-w-md text-center shadow-2xl">
                        ...
                        <p className="text-gray-700 mb-6 font-medium">
                            You navigated away from the exam window. This is strike {violationCount} of {maxViolations}. If you reach {maxViolations} strikes, your exam will be automatically submitted.
                        </p>
                        <button
            ...
            <div className="max-w-3xl mx-auto">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 mb-6 sticky top-4 z-40 flex justify-between items-center">
                    <div>
                        <h1 className="text-xl font-bold text-gray-800">{quizDetails?.title}</h1>
                        <p className="text-sm text-gray-500">Attempt ID: {attempt?._id.slice(-6)}</p>
                    </div>
                ...
                <div className="space-y-6">
                    {attempt?.questionsSnapshot.map((q, index) => {
                        const currentAnswer = answers.find(a => a.questionId === q.questionId);

                        return (
                            <div key={q.questionId} className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                                <div className="flex justify-between items-start mb-4">
                                    <h3 className="text-lg font-medium text-gray-900">
                                        <span className="text-purple-600 font-bold mr-2">Q{index + 1}.</span>
                                        {q.questionText}
                                    </h3>
                                ...
                                <div className="space-y-3 mt-4">
                                    {q.options.map((opt, optIndex) => (
                                        <label
                                            key={optIndex}
                                            className={`flex items-center p-4 border rounded-xl cursor-pointer transition-all ${currentAnswer?.selectedOptionIndex === optIndex
                                        ...
                                        >
                                            <input
                                                ...
                                                checked={currentAnswer?.selectedOptionIndex === optIndex}
