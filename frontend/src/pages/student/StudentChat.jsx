...
export default function StudentChat() {
        ...
        prev.map((c) =>
          c._id === chatId
            ? {
                ...
                unreadCount:
                  activeChatRef.current?._id === chatId
                    ? 0
                    : (c.unreadCount || 0) + 1,
              }
            : c,
  ...
  const handleSelectTutor = async (tutor) => {
    ...
    if (existing) {
      ...
      setChats((prev) =>
        prev.map((c) =>
          c._id === existing._id ? { ...c, unreadCount: 0 } : c,
        ),
      );
      return;
    }
    ...
    try {
      ...
      toast.error(
        err.response?.message || err.message || "Could not start chat",
      );
    } finally {
  ...
  const handleAcceptCall = async () => {
    ...
    try {
      ...
      if (!chat) {
        toast.error("Cannot find the chat for this call");
        return;
      }
  ...
  const handleEndCall = () => {
    if (socket && activeChat) {
      const other = getOtherParticipant(activeChat);
      if (other)
        socket.emit(SOCKET_EVENTS.END_CALL, { targetUserId: other._id });
    }
    setCallSession(null);
  };
  ...
  const StatusTick = ({ status, isMe }) => {
    if (!isMe) return null;
    if (status === "read")
      return <CheckCheck className="w-3.5 h-3.5 text-blue-300 flex-shrink-0" />;
    if (status === "delivered")
      return (
        <CheckCheck className="w-3.5 h-3.5 text-purple-300 flex-shrink-0" />
      );
          ...
          {loadingContacts ? (
            ...
            sidebarItems.map((item) => {
              const isActive =
                activeChat &&
                ((item.isExistingChat &&
                  activeChat._id === item.chatData?._id) ||
          ...
          <div className="bg-white px-5 py-3.5 flex items-center justify-between border-b border-purple-100 shadow-sm">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                title={
                  callRequested ? "Request already sent" : "Request video call"
                }
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <span
                          className={`text-[10px] ${isMe ? "text-purple-200" : "text-gray-400"}`}
                        >
