...
export const tutorAPI = {
  ...
  requestPasswordChange: () => api.post("/tutor/change-password/request"),

  verifyPasswordChange: (newPassword, otp) =>
    api.post("/tutor/change-password/verify", { newPassword, otp }),

  // Revenue
  getRevenueDashboard: () => api.get("/tutor/revenue"),
  ...
  // Categories
  getCategories: () =>
    api.get("/categories/public", { params: { limit: 100, _t: Date.now() } }),

  // Wallet
  getWallet: (params = {}) => api.get("/tutor/wallet", { params }),

  requestWithdrawal: (amount) => api.post("/tutor/wallet/withdraw", { amount }),
